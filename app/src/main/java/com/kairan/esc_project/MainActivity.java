...
public class MainActivity extends AppCompatActivity {
    // create reference to firebase and user, create wifi header

    FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
    DatabaseReference database = FirebaseDatabase.getInstance().getReference("WIFI").child(user.getUid());
                ...
                for (int i = 0; i < scanList.size(); i++) {
                    ...
                    database.addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            if (!snapshot.hasChild(name) && name.length()>1 ){database.child(name).setValue(rssi);}
                        }
